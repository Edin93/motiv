...
export default function SignIn(props) {
    ...
    const checkCredentials = async () => {
        ...
        try {
            const response = await axios.post('http://172.20.10.2:3000/api/users/login', {email, password});
            if ('user' in response.data) {
                const user = await axios.get(`http://172.20.10.2:3000/api/users/${response.data.user}`);
